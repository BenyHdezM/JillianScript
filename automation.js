...
(async () => {
  ...
  if ((await page.$("#Username")) !== null) {
    ...
    await Promise.all([
      page.waitForNavigation({ waitUntil: "domcontentloaded" }), // también puedes usar 'networkidle0' o 'domcontentloaded'
      await page.click(".btn.btn-login"),
    ]);
  }
  const prompt = require("prompt-sync")();
  const vCode = prompt("Type Verification Code:");
  console.log("Code: " + vCode);

  await page.waitForSelector("#verificationCode");
  await page.type("#verificationCode", vCode);
  ...
  for (let index = 2; index > 0; index++) {
    try {
      ...
      if (
        (await page2.$(`#BtnSave_Button`)) !== null &&
        (await page2.$(`#BillingProperty_LookupCode`)) !== null
      ) {
        ...
        const numInvetario = generarNumeroInventario(
          lookupCode,
          date,
          arrDate,
          chargeCode,
        ...
        //Charge To
        // await page2.waitForNetworkIdle({ waitUntil: 'load', idleTime: 1000, timeout: 0 });
        ...
        // Aquí puede ocurrir un reload automático
        await page2.waitForNavigation({
          waitUntil: "networkidle0",
          timeout: 0,
        });
        // Luego continúa
        await page2.waitForSelector("#ChargeNotes_TextBox", { visible: true });
        // await page2.click('#ChargeNotes_TextBox');
        await page2.type("#ChargeNotes_TextBox", notes);

        //Payable To
        await page2.type(
          "#BilledProperty_LookupCode",
          worksheet[`B${index}`].v.toLowerCase(),
        );
        ...
        //Charge Segment
        await page2.type(
          "#ChargeSegment1_LookupCode",
          lookupCode.toLowerCase(),
        );

        const messageContainer = await page2.$("#MessageTableContainer");
        if (messageContainer !== null) {
          const messageText = await page2.evaluate(() => {
            const el = document.querySelector("#MessageTableContainer");
            return el ? el.innerText.trim() : "";
          });

          const logMessage = `⚠️ Index: ${index} - Posible error o advertencia.\n`;
          fs.appendFileSync("console.log", logMessage);

          arrayConsoleLog.push(logMessage.trim());
        }

        // Finish - Save
        // await page2.waitForNetworkIdle({ waitUntil: 'domcontentloaded', idleTime: 1000, timeout: 0 });
        await page2.click("#Body1");
        // Luego continúa
        await page2.waitForSelector("#BtnSave_Button", { visible: true });
        page2.click("#BtnSave_Button");
        await page2.waitForNavigation({
          waitUntil: "networkidle0",
          timeout: 0,
        });
